...
fixes dominate architecture choice: a 10-epoch \emph{regularisation warm-up}
($+36.3$\,pp) and matching the model's frame count to the true clip length
($+4.4$\,pp). Once these are fixed, a controlled ablation study shows that
...
its backbone from ResNet18 to ResNet50 then adds a further $+3.4$\,pp
...
reaches \textbf{46.75\%} \texttt{val\_dir} and \textbf{0.5072} public /
\textbf{0.4764} private on Kaggle --- our best closed-track submission. In
...
\label{sec:arch}
All three Track~A architectures share the input/output contract
$(B, T, C, H, W) \to (B, 33)$ with $T{=}4$, $H{=}W{=}224$, and build on a
ResNet backbone~\cite{he2016resnet} that maps each $224{\times}224{\times}3$
frame to a $512$-d (ResNet18) or $2048$-d (ResNet50) feature after global
average pooling. They differ only in \emph{how} the $T$ per-frame feature
vectors are turned into a single $33$-way prediction --- the choice that
defines each model's parameter budget and its temporal inductive bias.

\noindent\textbf{CNNBaseline} and \textbf{CNN-LSTM} simply average (resp.\
recurrently combine) the $T$ pooled features; with four frames the LSTM gives
only marginal gains because backpropagation through time on such short
sequences is unstable. Both serve as lower bounds ($\approx\!11.2$\,M
parameters, ResNet18 backbone).

\noindent\textbf{TSM-ResNet}~\cite{lin2019tsm} wraps every residual block: a
fraction $1/\text{fold\_div}$ of channels is shifted one step backward and
another $1/\text{fold\_div}$ one step forward along the time axis,
\begin{equation}
  ...
  \label{eq:tsm}
\end{equation}
mixing temporal information at \emph{zero parameter cost} by re-indexing
tensor slices \emph{before} every convolution, while spatial resolution is
still full. After 8 such blocks, a $512$-d (R18) or $2048$-d (R50) per-frame
vector is produced, the $T$ vectors are averaged, and a linear head
$d\!\times\!33$ yields the logits. Total parameter count is therefore
\emph{exactly} that of the backbone plus the head:
$\approx\!11.2$\,M for ResNet18 ($P_\text{head}{=}512{\times}33{\approx}17$K)
and $\approx\!23.6$\,M for ResNet50 ($P_\text{head}{=}2048{\times}33{\approx}68$K,
of which $23.5$\,M sit in the backbone). TSM was designed for SSv2-style motion
labels and is therefore a strong prior here (Fig.~\ref{fig:arch}).

\noindent\textbf{VideoFormer-Lite (VFL)} instead separates spatial and temporal
processing into two explicit stages. \emph{Stage 1:} a ResNet18 processes each
frame independently, giving $T$ vectors of size $d{=}512$. \emph{Stage 2:} a
learnable \texttt{[CLS]} token~\cite{dosovitskiy2021vit} is appended (sequence
length $T{+}1{=}5$), and $n_\ell$ pre-norm Transformer
blocks~\cite{xiong2020prenorm} (8 heads, MLP ratio $r{=}4$) perform global
self-attention over the five tokens before the \texttt{[CLS]} representation is
classified. Each block costs
\begin{equation}
  ...
  \label{eq:vfl_params}
\end{equation}
i.e.\ $\approx\!3.1$\,M for $d{=}512,\,r{=}4$. With $n_\ell{=}3$ blocks the
Transformer head adds ${\approx}9.4$\,M on top of the ${\approx}11.2$\,M
ResNet18 backbone, for a total of ${\approx}20.7$\,M --- nearly twice
TSM-ResNet18, for \emph{zero} extra spatial capacity.
...
\emph{before} any pooling: after 8 blocks of depth, each frame's
representation already encodes a receptive field spanning all $T$ frames at the
$(h, w)$ level. VFL pools all spatial information \emph{first}, then attends
globally over the $T{+}1$ pooled vectors --- it sees motion only at the level
of frame-averaged descriptors. For SSv2, where the discriminative cue is often
a single finger or object edge displacing within a small region, this
distinction is consequential: TSM retains the spatial precision that the cue
lives in, whereas VFL has already discarded it by the time it reasons about
time (Fig.~\ref{fig:arch}). This architectural argument is exactly what the
controlled comparison in Sec.~\ref{sec:results} measures.
...
duplicates frames: for TSM, two consecutive identical tokens make
Eq.~\eqref{eq:tsm} read the same value twice, contributing zero temporal
gradient for the shifted channels and corrupting the very signal the
architecture is built around. Setting \texttt{num\_frames}\,$=$\,4 brought
\texttt{tsm\_ultra\_v2} to $57.73\%$ internal / $38.25\%$ \texttt{val\_dir}
($+4.41$ / $+3.34$\,pp); a first attempt to scale to ResNet50 \emph{at the old
$T{=}8$} regressed to $45.26\%$ internal (Fig.~\ref{fig:permodel},
Fig.~\ref{fig:backbone}), confirming that capacity cannot compensate for
corrupted temporal input. Once \texttt{num\_frames}\,$=$\,4 was enforced, a
second ResNet50 run (\texttt{tsm\_ultra\_resnet50\_rotating}, trained with
rotating folds, Sec.~\ref{sec:rotating}) reaches \textbf{42.85\%}
\texttt{val\_dir} --- our best single model, $+3.40$\,pp over the matched-data
ResNet18 (Sec.~\ref{sec:abl}). The lesson generalises: \emph{fix the data
pipeline before scaling the model}, and once the pipeline is correct, capacity
\emph{does} pay off.
...
into the loss and collapses representations. The stack also includes label
smoothing~\cite{muller2019labelsmooth}, RandAugment~\cite{cubuk2020randaugment},
...
concentrates gradient on confusable class pairs --- specifically the
real-vs-pretended sinks (Sec.~\ref{sec:failure}); our $\gamma$-sweep
(Sec.~\ref{sec:abl}) shows any focal weighting beats plain cross-entropy by
${\approx}0.5$--$0.6$\,pp, with $\gamma{\in}[1,2]$ all within noise of each
other. Augmentation is domain-aware: \emph{horizontal flip is disabled} because
it inverts SSv2 direction-encoded labels (e.g.\ \emph{Pulling
left$\rightarrow$right}); CutMix probability is kept low ($p{=}0.25$) because a
cut patch spans all four frames and frequently occludes the hand/object motion
...
The final closed-track submission fuses \emph{five} checkpoints by log-softmax
late fusion. Three use 10-crop spatial TTA (5 crops $\times$ 2 flips):
\texttt{tsm\_ultra\_v2} ($57.73\%$ internal), its rotating-fold variant, and
\texttt{video\_former\_lite\_ultra\_rot}. Two run without TTA (inference cost):
the new \textbf{TSM-ResNet50 rotating} ($42.85\%$, our best single model) and a
CE-trained ($\gamma{=}0$) TSM ablation that adds complementary calibration.
Averaging in log-probability space down-weights over-confident errors and
rewards agreement, suiting members of different calibration. The ensemble
reaches \textbf{46.75\%} on the official \texttt{val\_dir}
(Fig.~\ref{fig:ensemble}) and \textbf{0.5072} public / \textbf{0.4764} private
on Kaggle --- our best closed-track leaderboard score, $+9.1$\,pp public over
the strongest single-model submission (\texttt{tsm\_ultra\_v2} + TTA, $0.4163$;
Table~\ref{tab:summary}). 10-crop TTA alone contributes ${\sim}1$\,pp on the
strongest single model (Fig.~\ref{fig:tta}), with a visible trade-off on
direction-encoded classes (Fig.~\ref{fig:ttatradeoff}) where the 5 hflip views
push some confidences toward the mirror class even after label-aware logit
remapping.
...
Transformers~\cite{xiong2020prenorm}; the regularisation-warm-up recipe, the
rotating $k$-fold protocol, the controlled ablation suite (focal-$\gamma$, VFL
depth/fold-count/schedule sweeps), and the log-softmax ensemble / 10-crop TTA
pipeline; and the full Track~B stack---window-capped source-frame re-extraction,
the progressive-unfreezing\,+\,LLRD fine-tuning wrapper (following the
layer-wise decay practice of~\cite{howard2018ulmfit,clark2020electra,bao2022beit}),
and the noisy-student pseudo-labelling loop~\cite{xie2020noisystudent,lee2013pseudo}.
...
\paragraph{Overall.}
Table~\ref{tab:summary} reports headline results, now with Kaggle scores for
\emph{both} tracks. Track~A reaches \textbf{46.75\%} \texttt{val\_dir} /
\textbf{0.5072} Kaggle public from scratch; Track~B reaches \textbf{0.6586}
Kaggle top-1 (internal val $65.19\%$) with the 14-model uniform
ensemble---well above the $0.55$ provided baseline and approaching the $0.74$
second-place public reference. The ${\approx}15$\,pp Kaggle gap between the two
tracks' best submissions quantifies the practical value of large-scale
self-supervised pretraining, unavailable in the closed track; the gap is
markedly smaller than the ${\sim}18$-pp \texttt{val\_dir} gap because Track~A's
ensemble generalises comparatively better from \texttt{val\_dir} to the Kaggle
test distribution (see also the calibration discussion below).
...
A & \textbf{5-model ens.}\ (R50)& \textbf{46.75}\% & \textbf{0.5072} & \textbf{0.4764} \\
\midrule
...
At matched settings (rotating folds, $T{=}4$, ResNet18 backbone, no TTA),
TSM reaches $39.45\%$ \texttt{val\_dir} vs.\ VFL's $36.44\%$
($+3.01$\,pp). This is the controlled test of the architectural argument in
Sec.~\ref{sec:arch}: with backbone, frame count, recipe and protocol held
fixed, the only difference is \emph{where} the two models mix time --- before
pooling (TSM) or after (VFL) --- and the local-mixing model wins by a clear
margin, consistent with SSv2 labels being defined by small, spatially-localised
motion cues that pooling destroys.

\paragraph{Backbone scale-up, done right ($+3.4$\,pp).}
Once $T{=}4$ and rotating folds are fixed, replacing the ResNet18 backbone with
ResNet50 ($11.2\!\rightarrow\!23.6$\,M parameters, Sec.~\ref{sec:arch}) brings
$39.45\%\!\rightarrow\!42.85\%$ \texttt{val\_dir} ($+3.40$\,pp,
Fig.~\ref{fig:backbone}). Reading this together with the $T{=}8$ regression
above closes the loop on our central architectural claim: \emph{capacity is
useless on top of a corrupted temporal signal, but valuable once the signal is
correct} --- the $2.1\times$ parameter increase here converts cleanly into
accuracy, whereas the earlier $T{=}8$ run wasted the same increase on
duplicated frames.

\paragraph{Focal-loss $\gamma$ sweep ($+0.5$--$0.6$\,pp, optimum flat).}
With every other variable frozen (rotating folds, TSM-ResNet18), CE
($\gamma{=}0$) reaches $38.92\%$, $\gamma{=}1$ reaches $39.50\%$, and
$\gamma{=}2$ reaches $39.45\%$ (Fig.~\ref{fig:abla_tsm}). Any focal weighting
beats CE by ${\approx}0.5$\,pp; the $\gamma{=}1$ vs.\ $\gamma{=}2$ gap
($0.05$\,pp) is within run-to-run noise, so the optimal $\gamma$ lies in
$[1,2]$ and its exact value is not critical --- evidence that the gain comes
from \emph{having} a confusable-pair-aware loss, not from tuning it finely.
Despite scoring lowest standalone, the CE model contributes $+0.52$\,pp to the
5-model ensemble (Sec.~\ref{sec:rotating}): its different loss geometry yields
a different confidence calibration, which is exactly the kind of
complementary error the log-softmax fusion exploits.

\paragraph{VFL depth and fold count.}
Reducing VFL from 3 to 2 Transformer layers
(Eq.~\eqref{eq:vfl_params}: $P_\text{Transformer}$ drops from
$9.4\!\rightarrow\!6.3$\,M) costs $36.44\%\!\rightarrow\!35.36\%$
($-1.08$\,pp): the third block provides a real, if modest, gain at this data
scale (Fig.~\ref{fig:abla_vfl}). Increasing the rotation from 5 to 10 folds
(90\% vs.\ 80\% of the data seen per epoch) instead \emph{costs} $-0.46$\,pp
($36.44\!\rightarrow\!35.98\%$): VFL has already saturated its
data-utilisation capacity at $k{=}5$, so additional coverage trades off against
noisier gradient estimates from smaller per-fold validation sets. Switching the
SGDR restart shape ($T_\text{mult}\,1\!\rightarrow\!2$) changes accuracy by at
most $-0.12$\,pp for either architecture --- the rotating-fold protocol already
supplies enough implicit regularisation that the exact LR-restart geometry is
largely irrelevant.

\paragraph{Ensemble components and leave-one-out.}
\label{sec:loo}
Table~\ref{tab:loo} (Fig.~\ref{fig:loo}) shows the leave-one-out analysis on
the final 5-model ensemble (46.75\%). Every member is necessary, but the
\emph{margins} are revealing: removing the ResNet50 model costs
$-1.33$\,pp --- by far the largest single drop, larger than removing any
same-backbone TSM-ResNet18 variant ($-0.31$ to $-0.47$\,pp) or even the
architecturally-different VFL ($-0.64$\,pp). \textbf{Backbone diversity
contributes more to the ensemble than architecture-family diversity at matched
capacity.} This refines the earlier (3-model) finding that TSM and VFL are
complementary: that remains true, but the single largest source of
decorrelated, useful error in our pool is a model that simply \emph{sees more}
(deeper backbone), not one that sees \emph{differently}.
...
away the very decorrelation that the uniform ensemble exploits. This mirrors,
at a larger scale, our Track~A finding (Sec.~\ref{sec:loo}) that the most
valuable ensemble member is the one that is most \emph{different}, not the one
that is individually strongest.
...
iterations. By contrast, Track~A's best ensemble has a \emph{negative} gap
($46.75\%$ \texttt{val\_dir} vs.\ $0.5072$ Kaggle public, i.e.\ Kaggle is
\emph{higher}): a from-scratch model trained with heavy augmentation is, if
anything, slightly better calibrated to an unseen test distribution than to its
own (smaller, finite) validation set, whereas a fine-tuned pretrained model can
specialise to \texttt{val\_dir} quirks. The train-split$\rightarrow$Kaggle gap
for Track~B (${\approx}0.09$) reflects that the internal split is drawn from
the training distribution while the Kaggle test set is sampled independently.
...
\textbf{0.5072} on the public Kaggle leaderboard, with leave-one-out analysis
...
(1)~Running 10-crop TTA for TSM-ResNet50 (expected ${\sim}{+}2$\,pp based on the
ResNet18 measurement, currently impractical at submission time due to inference
cost); (2)~V-JEPA-Giant once inference-time memory is addressed; (3)~targeted
class rebalancing for the \emph{holding}/\emph{pretending} sinks in both tracks;
(4)~temperature-scaled confidence thresholds for a second, less over-fitting
pseudo-label round; (5)~a 15th Track~B ensemble member from a 16-frame
window-capped VideoMAE-Large checkpoint, completed after the leaderboard freeze,
yielded $0.6592$ (within noise of $0.6586$, confirming returns have flattened);
(6)~explicit trajectory-prediction heads to attack the real-vs-pretended
ambiguity directly; (7)~applying the $T{=}4$ fix and rotating-folds protocol to
the Track~B TSM family, whose current weakness ($0.3364$ \texttt{val\_dir})
likely stems from the same frame-count issue that cost Track~A $4.4$\,pp.
...
analysis, the ResNet50 backbone scale-up, the controlled ablation suite
(architecture, focal-$\gamma$, VFL depth/folds/schedule), and the log-softmax
ensemble/TTA code, and produced the Track~A ablation and per-class figures.
\textbf{Florian Guillaumey} designed and implemented the full Track~B
(Open-World) pipeline: the window-capped source-frame re-extraction, the
V-JEPA\,2 fine-tuning wrapper with progressive unfreezing and LLRD, the
pseudo-labelling stage, the honest cross-preprocessing 14-model ensemble, and
the data-integrity audit. Both authors jointly analysed results and wrote this
report.
...
\begin{figure}[h]
  ...
  \texttt{val\_dir} and $0.5072$ public / $0.4764$ private on Kaggle ---
